...
class Hub:
    ...
    * Use ``Ctrl + C`` for exiting.
    ...
    def worker(self, worker_class, name: str, args: Tuple = (), kwargs=None):
        ...
        process = CTX.Process(
            name=name,
            target=self._build_worker_and_run,
            args=(worker_class, name, *args),
            kwargs=kwargs,
        )
    ...
    def run(self):
        ...
        ######################## Replace this because doesn't work anymore
...
class PushListener(Worker):
    ...
    def _parse(self, msg: str) -> List:
        ...
        for index, payload in enumerate(payloads):
            ...
            assert (
                len(words) > 1
            ), f"unexpected payload at index {index}, context: {msg}"
            ...
            assert (
                current_push_type is not None
            ), f"can not decide push type of payload at index {index}, context: {msg}"
            ...
            if current_push_type == self.PUSH_TYPE_CHASSIS:
                ...
                raise ValueError(
                    f"unknown push type {current_push_type} at index {index}, context: {msg}"
                )
        return parsed
    ...
    def _parse_chassis_push(words: List[str], has_type_prefix: bool):
        ...
        if subtype == "position":
            ...
            raise ValueError(
                f"unknown chassis push subtype {subtype}, context: {words}"
            )
...
class EventListener(Worker):
    ...
    def _parse(self, msg: str) -> List:
        ...
        for index, payload in enumerate(payloads):
            ...
            ), f"can not decide event type of payload at index {index}, context: {msg}"
            ...
            if current_event_type == self.EVENT_TYPE_ARMOR:
                ...
                raise ValueError(
                    f"unknown event type {current_event_type} at index {index}, context: {msg}"
                )
        return parsed
